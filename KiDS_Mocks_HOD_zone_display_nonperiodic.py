...
numpart, numzones, zone = read_zone('KiDS_Mocks_0_525_noncube.zone')
numpart_vol, vol, non_zero_idx = read_vol('KiDS_Mocks_0_525_noncube.vol.txt', vol_cone)
x,y,z = read_AllHalo('KiDS_Mocks_gal_pos_0_525_v2.txt')
# x,y,z = read_inner('zobov_poisson_test/inner.pos')
ID, x_vol, y_vol, z_vol, x_denmin, y_denmin, z_denmin, zone_rad, dencon = read_vol_zone_txt('KiDS_Mocks_0_525_noncube.vol.zone.txt')
adj_dict = read_adj('KiDS_Mocks_0_525_noncube.adj.txt')

# Zone per particle that has a non zero volume
zone_nonzero = [zone[q] for q in non_zero_idx]
void_idx = rm_duplicate(zone_nonzero)
...
def spherical_stk(zone_rad_stk,x_zone_stk,y_zone_stk,z_zone_stk,numzone_stk):
	for r in range(0,(len(zone_rad_stk))):
		...
		for n in range(0,len(R_stk)):
			...
			if n==0:
				count_temp1 = len(tree.query_ball_point([x[ind_x_stk], y[ind_y_stk], z[ind_z_stk]], R_stk[n]*zone_rad_stk[r]))
				...
				den_temp1 = ((1./sum(vol_temp1))*count_temp1)/(numpart_vol/vol_cone)

			else:
				delta_r = (R_stk[n-1]*zone_rad_stk[r]) - (R_stk[n]*zone_rad_stk[r])
				count_temp11 = len(tree.query_ball_point([x[ind_x_stk], y[ind_y_stk], z[ind_z_stk]], R_stk[n]*zone_rad_stk[r]))
				count_temp12 = len(tree.query_ball_point([x[ind_x_stk], y[ind_y_stk], z[ind_z_stk]], R_stk[n-1]*zone_rad_stk[r]))
	...
	return avg_cnt, avg_den #,avg_nden
...
# cell_on_boundary_tot_2303, cell_on_boundary_2303, x_non_zone_adj_2303, y_non_zone_adj_2303, z_non_zone_adj_2303, adj_cell_vol_2303 = adj_particles(same_zone_adj,2303)
...
for i in range(0,len(zone_bins)-1):
	if zone_rad_stk[i] != []:
		avg_count_sph[i], avg_den_sph[i] = spherical_stk(zone_rad_stk[i],x_vol_stk[i],y_vol_stk[i],z_vol_stk[i],num_zn_stk[i])
# 		np.save('sph_count_nden_Mocks_v2_%d' % zone_bins[i+1], (avg_count_sph[i], avg_den_sph[i]))
...
# for i in range(0,len(zone_bins)-1):
# 	if zone_bins[i+1] < 10:
# 		avg_count_sph[i], avg_den_sph[i] = np.load('sph_count_nden_Mocks_0_525_noncube_0%d.npy' % zone_bins[i+1])	
# 	else:
# 		avg_count_sph[i], avg_den_sph[i] = np.load('sph_count_nden_Mocks_0_525_noncube_%d.npy' % zone_bins[i+1])
...
print 'zone radius from file', zone_rad[np.int(zone_nonzero[arb_ind])]
...
# ax1.set_xlim(x_vol[np.int(zone_nonzero[arb_ind])]-150,x_vol[np.int(zone_nonzero[arb_ind])]+150)
# ax1.set_ylim(y_vol[np.int(zone_nonzero[arb_ind])]-150,y_vol[np.int(zone_nonzero[arb_ind])]+150)
...
# circ2=plt.Circle((x_denmin[np.int(zone_nonzero[arb_ind])],y_denmin[np.int(zone_nonzero[arb_ind])]),zone_rad[np.int(zone_nonzero[arb_ind])], fill=None)
...
# ax1.scatter(x_denmin[np.int(zone_nonzero[arb_ind])],y_denmin[np.int(zone_nonzero[arb_ind])], color='black', marker='d', s=100)
# ax1.scatter(x_vol[np.int(zone_nonzero[arb_ind])],y_vol[np.int(zone_nonzero[arb_ind])], color='black', marker='+', s=100)
# ax1.add_artist(circ)
...
for i in range(0,len(zone_bins)-1):
	# if zone_rad_stk[i] != []:
	ax3.plot(R_stk, (np.array(avg_den_sph[i])/tot_numden), linewidth=3, label='%2d' % zone_bins[i+1], linestyle = next(linecycler))
# ax3.plot(R_stk, (np.array(avg_den_sph[1])/tot_numden), linewidth=3, label='%2d' % zone_bins[2])
# ax3.plot(R_stk, (np.array(avg_den_sph[2])/tot_numden), linewidth=3, label='%2d' % zone_bins[3])
# ax3.plot(R_stk, (np.array(avg_den_sph[3])/tot_numden), linewidth=3, label='%2d' % zone_bins[4])
...
# ax5.set_xlim(x_vol[np.int(zone_nonzero[arb_ind])]-50,x_vol[np.int(zone_nonzero[arb_ind])]+50)
# ax5.set_ylim(y_vol[np.int(zone_nonzero[arb_ind])]-50,y_vol[np.int(zone_nonzero[arb_ind])]+50)
...
# ax5.scatter(x_denmin[np.int(zone_nonzero[arb_ind])],y_denmin[np.int(zone_nonzero[arb_ind])], color='black', marker='d', s=100)
# ax5.scatter(x_vol[np.int(zone_nonzero[arb_ind])],y_vol[np.int(zone_nonzero[arb_ind])], color='black', marker='+', s=100)
...
# fig6.savefig('Boundary_prof_stk_bin_test_8_non0cnt', format='pdf')
